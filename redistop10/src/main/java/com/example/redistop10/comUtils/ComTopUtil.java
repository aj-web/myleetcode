...
public class ComTopUtil {
    ...
    public static List<ComDto> ComTopGet(String key, int size) {
        ...
        try {
            Set<ZSetOperations.TypedTuple<String>> typedTuples = getRedisTemplate().opsForZSet()
                    .reverseRangeWithScores(key, 0, size - 1);
            if (!CollectionUtils.isEmpty(typedTuples)) {
                comDtos = typedTuples.stream()
                        .map(stringTypedTuple -> ComDto.builder().value(stringTypedTuple.getValue())
                                .score(stringTypedTuple.getScore()).build()).sorted((c1, c2) -> (int) -(-c2.getScore()))
